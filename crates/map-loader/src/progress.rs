...
///   configurable interval so log files stay readable without bar-rewriting noise.
///
/// Two factory functions cover all current use-cases:
/// - [`make_count_bar`] — `{pos}/{len}` style, for polygon simplification / rasterization.
...
/// - `item_label`   — noun used in the bar template and fallback `println!` messages.
...
impl DownloadProgress {
    ...
    ///
    /// In non-TTY mode this emits a log line each time the downloaded amount
    /// crosses the next `log_interval_bytes` boundary.
...
///
/// - `total`              — total expected bytes, or `None` if unknown (renders as a spinner).
